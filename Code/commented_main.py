...
class DefinicoesClassificador:
    ...
    src_path_original = glob.glob(src_path + "/*/Original/*") # Diretorio das images de treino originais
    src_path_perfect = glob.glob(src_path + "/*/Perfect/*") # Diretorio das images de treino perfeitas
    src_path_results = "./TestImages/Results/Images/" # Diretorio das images de treino resultantes
    src_path_results_text = "./TestImages/Results/Text/" # Diretorio do ficheiro .txt com informação relativa ao teste
...
def crossover(populacao: list):
    ...
    for i in populacao:
        ...
        rand = populacao[pos]
        g1_a, g1_b = list(i[0]), list(i[2]) # Criar listas com os valores de cima e de baixo do array g1
        g2_a, g2_b = list(rand[0]), list(rand[2]) # Criar listas com os valores de cima e de baixo do array g2
        ...
        new_pop.append(new_gene1) # Adcionar o gene 1 ao array da nova população
        new_pop.append(new_gene2) # Adcionar o gene 2 ao array da nova população
...
def random_mutation(populacao: list):
    ...
    for i in random_population_alter:
        ...
        rnd = random.randint(0, 5) # Escolher aleatoriamente um elemento da lista
        ...
        # Adicionar os novos genes ao array dos novos genes
...
def apply_genoma(genoma, g_normals, lista_genomas, lista_distancias, imagem_float, imagem_perfeita_float):
    ...
    gX = scipy.ndimage.convolve(imagem_float, hX) # Convolução da imagem pelo genoma
    gY = scipy.ndimage.convolve(imagem_float, hY) # Convolução da imagem pela transposição do genoma
    g = np.abs(gX + gY) # Obter o valor absoluto da soma dos arrays trabalhados
...
if __name__ == '__main__':
    ...
    lista_de_treino = DefinicoesClassificador.src_path_original # Lista com as fotos originais
    iterations = 10 # Número de iterações a ser realizadas
    minimum = "None"
    populacao = gerar_populacao(10) # Definição do tamanho da população e sua criação
    ...
    total_img = len(lista_de_treino) # Numero total de imagens a ser processadas
    imagem_raw = []
    imagem_float = []
    imagem_perfeita = []
    imagem_perfeita_float = []
    ...
    for ficheiro_de_imagem in lista_de_treino:
        lista_path.append(ficheiro_de_imagem.split('\\')) # Lista para divir o diretorio de cada ficheiro um array
        nome.append(lista_path[-1][-1]) # Buscar o valor do array que represneta o nome do ficheiro
